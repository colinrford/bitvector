/*
 *  bench_compile_custom.cpp – written by Colin Ford
...
constexpr auto run_sieve = []() {
  bitvector<> bv(compile_time_size);
...
constexpr std::uint64_t get_expected_count(std::uint64_t n) {
  if (n == 100'000) return 9592;
  if (n == 262'144) return 23000;
  if (n == 524'288) return 43389;
  if (n == 1'000'000) return 78498;
  if (n == 2'000'000) return 148933; // From external knowledge/Wolfram
  if (n == 3'000'000) return 216816; 
  if (n == 4'000'000) return 283146;
  if (n == 5'000'000) return 348513; 
  if (n == 6'000'000) return 412849;
  if (n == 7'000'000) return 476648;
  if (n == 8'000'000) return 539777;
  if (n == 9'000'000) return 602489;
  if (n == 10'000'000) return 664579;
...
constexpr auto verify_count = []() {
  ...
  if (expected != 0) {
    if (actual != expected) throw "Prime count mismatch";
  }
  return actual;
};
