...
void test_big_int_roundtrip()
{
  ...
  if (bv.size() != EXPECTED_BITS)
    throw std::runtime_error(std::format("Size mismatch: expected {}, got {}", EXPECTED_BITS, bv.size()));
  ...
  if (original != recovered)
  {
    ...
    std::println("Original:  {:016x} {:016x} {:016x} {:016x}", original[0], original[1], original[2], original[3]);
    std::println("Recovered: {:016x} {:016x} {:016x} {:016x}", recovered[0], recovered[1], recovered[2], recovered[3]);
...
void test_zq_element_conversion()
{
  ...
  // ZqElement usually has an explicit operator to get the big_int data or just exposes .data
  // Based on user snippet: explicit operator auto() const { return data; }
  // We can convert to big_int first, OR if ZqElement exposes a range interface (it has .data member which is big_int)
  // user snippet said: "big_int<sizeof...(Modulus), T> data;"
