...
struct stateful_allocator
{
  ...
  friend constexpr bool operator==(const stateful_allocator& a, const stateful_allocator& b) { return a.id == b.id; }
};
...
int main()
{
  ...
  if (uninit_bv.size() != 50)
    return 1;
  if (uninit_bv.get_allocator().id != 202)
    return 1;
